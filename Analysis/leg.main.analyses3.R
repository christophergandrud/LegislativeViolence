...
D2 <- zelig(violence ~ HighProp + DemAge + maj + immunity + tenshort + pr, model = "relogit", data = dem.2.c, tau = 69/3347, robust = list(method = "weave"), cite = FALSE)
...
D4 <- zelig(violence ~ HighProp + DemAge + maj + enps + system + federal + govfrac, model = "relogit", data = dem.4.c, tau = 69/3347, robust = list(method = "weave"), cite = FALSE)
...
DN2 <- zelig(violence ~ HighProp + DemAge + maj + immunity + tenshort + pr, model = "relogit", data = dNew.2.c, tau = 61/2631, robust = list(method = "weave"), cite = FALSE)
...
DN4 <- zelig(violence ~ HighProp + DemAge + maj + enps + system + federal + govfrac, model = "relogit", data = dNew.4.c, tau = 61/2631, robust = list(method = "weave"), cite = FALSE)
